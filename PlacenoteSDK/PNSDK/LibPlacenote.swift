...
class LibPlacenote {
  typealias PoseCallback = (_ outputPose: matrix_float4x4, _ arkitPose: matrix_float4x4)-> Void
  typealias SaveMapCallback = (_ mapId: String?) -> Void
  typealias FileTransferCallback = (_ completed: Bool, _ faulted: Bool, _ percentage: Float) -> Void
  typealias DeleteMapCallback = (_ deleted: Bool) -> Void
  typealias ListMapCallback = (_ success: Bool, _ mapList: [String]) -> Void
   ...
   */
  func getMappingStatus () -> MappingStatus {
   ...
   */
  func getTrackedLandmarks() -> Array<PNFeaturePoint> {
   ...
   */
  func getAllLandmarks() -> Array<PNFeaturePoint> {
    var pointArray: [PNFeaturePoint] = []
    let feature: PNFeaturePoint = PNFeaturePoint()
  ...
  /**
   ...
   inertial map frame that LibPlacenote generated within this mapping session
   */
  func setFrame(image: CVImageBuffer, pose: matrix_float4x4) -> Void {
   ...
   */
  func saveMap(savedCb: @escaping SaveMapCallback, uploadProgressCb: @escaping FileTransferCallback) -> Void {
   ...
   */
  func deleteMap(mapId: String, deletedCb : @escaping DeleteMapCallback) {
    let cbCtx: CallbackContext = CallbackContext(id: UUID().uuidString, ptr: self)
  ...
  /**
   ...
   - Parameter deletedCb: async callback to indicate whether the map is successfully deleted
   */
  func loadMap(mapId: String, downloadProgressCb : @escaping FileTransferCallback) {
    let cbCtx: CallbackContext = CallbackContext(id: UUID().uuidString, ptr: self)
   ...
   */
  func fetchMapList(listCb: @escaping ListMapCallback) {
    let cbCtx: CallbackContext = CallbackContext(id: UUID().uuidString, ptr: self)
