...
class ViewController: UIViewController, ARSCNViewDelegate, ARSessionDelegate, PNDelegate {
    ...
    @IBAction func addChair(_ sender: Any) {
        ...
        modelManager.addModelAtPose(pos: reticle.getReticlePosition(), rot: reticle.getReticleRotation() ,index: 0)
    }
    
    @IBAction func addPlant(_ sender: Any) {
        
        print (String (describing: reticle.getReticlePosition()))
        modelManager.addModelAtPose(pos: reticle.getReticlePosition(), rot: reticle.getReticleRotation() , index: 1)
    }
    
    @IBAction func addLamp(_ sender: Any) {
        modelManager.addModelAtPose(pos: reticle.getReticlePosition(), rot: reticle.getReticleRotation(), index: 2)
    }
    
    @IBAction func addGramophone(_ sender: Any) {
        modelManager.addModelAtPose(pos: reticle.getReticlePosition(), rot: reticle.getReticleRotation(), index: 3)
    }
        ...
        LibPlacenote.instance.saveMap(
            savedCb: {(mapId: String?) -> Void in
                if (mapId != nil) {
                    ...
                    metadata.location!.latitude = 0
                    metadata.location!.longitude = 0
                    metadata.location!.altitude = 0
